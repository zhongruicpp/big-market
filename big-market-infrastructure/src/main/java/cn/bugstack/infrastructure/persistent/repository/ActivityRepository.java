...
public class ActivityRepository implements IActivityRepository {
    ...
    public void doSaveOrder(CreateQuotaOrderAggregate createOrderAggregate) {
        ...
        try {
            ...
            transactionTemplate.execute(status -> {
                try {
                    // 1. 写入订单 向 'raffle_activity_order' 中插入数据
    ...
    public boolean subtractionActivitySkuStock(Long sku, String cacheKey, Date endDateTime) {
        ...
        // 2. 加锁为了兜底，如果后续有恢复库存，手动处理等【运营是人来操作，会有这种情况发放，系统要做防护】，也不会超卖。因为所有的可用库存key，都被加锁了。1
        // 3. 设置加锁时间为活动到期 + 延迟1天
        ...
        try {
            ...
            transactionTemplate.execute(status -> {
                try {
                    ...
                    // 4. 写入参与活动订单 在 'user_raffle_order' 中进行插入
    ...
    public void updateOrder(DeliveryOrderEntity deliveryOrderEntity) {
        ...
        try {
            ...
            transactionTemplate.execute(status -> {
                try {
                    // 1. 更新订单 在 'raffle_activity_order'中写入数据
    ...
    public UnpaidActivityOrderEntity queryUnpaidActivityOrder(SkuRechargeEntity skuRechargeEntity) {
        ...
        if (raffleActivityOrderRes == null) return null;
        return UnpaidActivityOrderEntity.builder()
                ...
                .payAmount(raffleActivityOrderReq.getPayAmount())
                .build();
    }

    @Override
    public List<SkuProductEntity> querySkuProductEntityListByActivityId(Long activityId) {
        // 在 'raffle_activity_sku' 中进行查询
        List<RaffleActivitySku> raffleActivitySkus = raffleActivitySkuDao.queryActivitySkuListByActivityId(activityId);
